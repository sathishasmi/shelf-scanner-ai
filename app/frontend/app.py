...
st.write(
    "Take a picture of any bookshelf to get instant, personalized reading recommendations!"
)
...
uploaded_file = st.file_uploader(
    "Upload or snap a photo of a bookshelf", type=["jpg", "jpeg", "png"]
)
...
if uploaded_file is not None:
    ...
    if st.button("Scan Bookshelf ", type="primary"):
        if not user_pref.strip():
            ...
            with st.spinner(
                "AI is examining the spines and curating your reading list..."
            ):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.error(
                            f"Server returned an error status code: {response.status_code}"
                        )

                except requests.exceptions.ConnectionError:
                    st.error(
                        "Could not connect to the Backend server. Make sure your FastAPI app is running on port 8000."
                    )
